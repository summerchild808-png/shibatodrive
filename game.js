...
//  game.js  Part 1 (前半・ルート分岐進捗バグ修正版)
// ===============================
...
let gameState = 'TITLE';
let currentStage = 1;      // 現在の下道ステージ管理（1:福岡, 2:広島, 3:大阪, 4:横浜渋谷）
let lastPassedStage = 0;   // 直前にクリアしたステージを記憶
  ...
  function processInputStart(canvasX, canvasY, buttonNum = 0) {
    ...
    if (gameState === 'TITLE') {
      ...
      if (saveData) {
        if (canvasX >= 140 && canvasX <= 440 && canvasY >= 340 && canvasY <= 410) {
          ...
          currentStage = saveData.stage; lastPassedStage = saveData.stage - 1; scoreMoney = saveData.money; gameState = 'SELECT_ROUTE';
        }
      } else {
    ...
    if (gameState === 'STAGE_CLEAR' && endCardTimer >= END_CARD_DELAY) {
      if (currentStage === 4) {
        ...
        // クリアしたステージを記憶して、次のステージ番号を進めておく
        if (currentStage !== 99) {
          lastPassedStage = currentStage;
          currentStage++;
        } else {
          // 高速道路（99）クリア時は、前のステージの次のステージへ進める
          lastPassedStage++;
