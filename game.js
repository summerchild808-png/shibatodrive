...
//  game.js  Part 1 (前半・チュートリアル時完全停止対応版)
// ===============================
...
function init() {
  ...
  function processInputStart(canvasX, canvasY, buttonNum = 0) {
    ...
    if (gameState === 'PLAYING') {
      ...
      else if (canvasX <= 220 && canvasY >= 230 && canvasY <= 335) {
        moveLane(-1);
      }
      else if (canvasX <= 220 && canvasY >= 345 && canvasY <= 450) {
        moveLane(1);
      }
      return;
    }
...
function gameLoop() {
  // 🛑 チュートリアルダイアログ表示中も更新をストップ！
  if (!isPaused && !showInitialTutorial) update();
