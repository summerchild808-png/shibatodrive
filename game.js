...
function init() {
  ...
  function processInputStart(canvasX, canvasY, buttonNum = 0) {
    ...
    // タイトル画面のボタン判定（座標は後半パートの描画と一致させています）
    if (gameState === 'TITLE') {
  ...
  document.addEventListener('keydown', e => {
    ...
    else if (e.code === 'ArrowDown' || e.code === 'KeyS') moveLane(Dirdir);
    else if (e.code === 'ArrowDown' || e.code === 'KeyS') moveLane(Dirdir);
...
function draw() {
  ...
  // ===============================
    ...
    } else if (currentStage === 4) { // 🏙️ 横浜・渋谷
