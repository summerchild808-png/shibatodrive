...
//  game.js  Part 1 (前半・高速完走エンディング対応版)
// ===============================
...
let gameState = 'TITLE';
let currentStage = 1;
let lastPassedStage = 0; // 通過済みの最新下道ステージ（1:福岡クリア, 2:広島クリア, 3:大阪クリア）
...
function init() {
  ...
  function processInputStart(canvasX, canvasY, buttonNum = 0) {
    ...
    if (gameState === 'TITLE') {
      ...
      if (saveData) {
        if (canvasX >= 140 && canvasX <= 440 && canvasY >= 340 && canvasY <= 410) {
          clearSaveData(); scoreMoney = 0; currentStage = 1; lastPassedStage = 0; gameState = 'STORY';
        } else if (canvasX >= 520 && canvasX <= 820 && canvasY >= 340 && canvasY <= 410) {
          currentStage = saveData.stage; lastPassedStage = saveData.lastPassed || (saveData.stage - 1); scoreMoney = saveData.money; gameState = 'SELECT_ROUTE';
        }
      } else {
        scoreMoney = 0; currentStage = 1; lastPassedStage = 0; gameState = 'STORY';
      }
      return;
    }
    ...
    // 🎬 ステージクリア後の進行処理（高速での最終完走判定を追加！）
    if (gameState === 'STAGE_CLEAR' && endCardTimer >= END_CARD_DELAY) {
      // 下道で渋谷（4）をクリア、または高速走行で大阪以降（lastPassedStage >= 3）から完走した場合 ➔ エンディングへ！
      if (currentStage === 4 || (currentStage === 99 && lastPassedStage >= 3)) {
