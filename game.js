...
function init() {
  ...
  function processInputStart(canvasX, canvasY, buttonNum = 0) {
    // ⏸️ ポーズボタン（判定を広めに拡張: X: 720〜820, Y: 0〜80）
    ...
    // 🕹️ 下道プレイ中の操作（判定エリアを大幅拡大！）
    if (gameState === 'PLAYING') {
      // 🦘 JUMPボタン（右下広範囲: X > 700 ＆ Y > 240）
      if (canvasX > 700 && canvasY > 240) {
        handleAction();
      } 
      // ▲ 上ボタン（左側上部: X <= 220 ＆ Y: 240〜355）
      else if (canvasX <= 220 && canvasY >= 240 && canvasY < 355) {
        moveLane(-1);
      } 
      // ▼ 下ボタン（左側下部: X <= 220 ＆ Y >= 355）
...
// ===============================
//  忠犬しばとのお迎え大作戦（横画面・PNG統一版）
//  game.js  Part 2 (後半：怪獣レイヤー福岡タワー同期版)
// ===============================
