...
let showInitialTutorial = false; // 🔰 初回チュートリアルダイアログ用
...
function init() {
  ...
  function processInputStart(canvasX, canvasY, buttonNum = 0) {
    ...
    if (gameState === 'STORY') {
      resetStage(1);
      showInitialTutorial = true; // 初回ステージ開始時にチュートリアル表示
      gameState = 'PLAYING';
      return;
    }

    // 🏁 ステージクリア画面（下道4 or 高速99完走でエンディング直行）
  ...
  document.addEventListener('keydown', e => {
    ...
    if (gameState === 'PLAYING' && showInitialTutorial && (e.code === 'Space' || e.code === 'Enter')) {
      showInitialTutorial = false;
    }
...
function update() {
  ...
  if (gameState === 'PLAYING' && !showInitialTutorial) updateStageLogic();
...
function draw() {
  ...
  if (gameState === 'PLAYING' && showInitialTutorial) {
    ...
    ctx.beginPath(); ctx.roundRect(160, 40, 640, 360, 20); ctx.fill(); ctx.stroke();

    ctx.textAlign = 'center'; ctx.fillStyle = '#FFD700'; ctx.font = 'bold 24px sans-serif';
    ctx.fillText('【 🚘 お迎え大作戦の基本ルール 】', canvas.width / 2, 80);

    ctx.fillStyle = '#FFF'; ctx.font = '16px sans-serif'; ctx.textAlign = 'left';
    let startX = 200;
    
    ctx.fillStyle = '#00FF7F';
    ctx.fillText('⭕ 拾おう！', startX, 125);
    ctx.fillStyle = '#FFF';
    ctx.fillText('・ヒッチハイカー / メカニック（乗車・点検でお金ゲット！）', startX + 100, 125);

    ctx.fillStyle = '#FF6B6B';
    ctx.fillText('❌ 避けよう！', startX, 175);
    ctx.fillStyle = '#FFF';
    ctx.fillText('・怪獣 / トラック / 乗用車（ジャンプや車線変更で回避！）', startX + 100, 175);

    ctx.fillStyle = '#FF0000';
    ctx.fillText('💥 危険！', startX, 225);
    ctx.fillStyle = '#FFF';
    ctx.fillText('・ダイナマイト（触れると即大爆発！！絶対に避けて！）', startX + 100, 225);

    ctx.fillStyle = '#FFD700';
    ctx.fillText('🎮 操作：画面左の「▲▼ボタン」で車線変更、「JUMP」でジャンプ！', startX, 275);

    ctx.fillStyle = '#00FF7F'; ctx.strokeStyle = '#FFF'; ctx.lineWidth = 2;
    ctx.beginPath(); ctx.roundRect(300, 310, 360, 60, 12); ctx.fill(); ctx.stroke();
