...
//  game.js  Part 1 (前半・高速チュートリアル強制表示修復版)
// ===============================
...
function init() {
  ...
  function processInputStart(canvasX, canvasY, buttonNum = 0) {
    ...
    // 🏎️ 高速道路チュートリアルの消去（「OK！発進する」タップ判定範囲）
...
// ===============================
//  game.js Part 1 (前半・下ボタン判定エリア引き上げ版)
// ===============================

// --- (画像読み込み〜変数定義部分は変更なし) ---

    // 🎮 プレイ中ボタン判定（ブラウザバー干渉回避：判定エリア上移動）
    if (gameState === 'PLAYING') {
      // 🦘 右下 JUMPボタン
      if (canvasX >= 760 && canvasY >= 240) {
        handleAction();
      }
      // ▲ 上ボタン（Y: 200〜295）
      else if (canvasX <= 220 && canvasY >= 200 && canvasY <= 295) {
        moveLane(-1);
      }
      // ▼ 下ボタン（Y: 305〜400 / 上方向に移動して干渉を回避）
      else if (canvasX <= 220 && canvasY >= 305 && canvasY <= 400) {
        moveLane(1);
      }
      return;
    }
...
// 🏎️ 高速リセット処理（ここで確実にチュートリアルフラグをONにする！）
function resetHighway() {
  ...
  popups = [];
  patrolCarX = -300;
  showHighwayTutorial = true; // 確実に表示ON
...
function gameLoop() {
  // 🛑 チュートリアルダイアログ（下道・高速）表示中はゲームの動きを完全ストップ！
...
// ===============================
//  忠犬しばとのお迎え大作戦（横画面・PNG統一版）
//  game.js  Part 2 (後半・高速チュートリアル描画修復版)
// ===============================
...
function draw() {
  ...
  // ===============================
// ===============================
//  game.js Part 2 (後半・下ボタン描画位置上移動版)
// ===============================

  // ===============================
  //  📱 UI描画（ブラウザバー回避：ボタン位置を上へシフト）
  // ===============================
  if (gameState === 'PLAYING' || gameState === 'PLAYING_HIGHWAY') {
    ...
    // 📱 ▲ 上ボタン（位置を上にシフト: Y:210, 130x70）
    ctx.save();
    ctx.fillStyle = 'rgba(0, 150, 255, 0.45)'; ctx.strokeStyle = '#FFF'; ctx.lineWidth = 3;
    ctx.beginPath(); ctx.roundRect(15, 210, 130, 70, 14); ctx.fill(); ctx.stroke();
    ctx.fillStyle = '#FFF'; ctx.font = 'bold 26px sans-serif'; ctx.textAlign = 'center';
    ctx.fillText('▲ 上', 80, 253);

    // 📱 ▼ 下ボタン（ブラウザバーと離すため Y:315 へ上移動！）
    ctx.fillStyle = 'rgba(0, 150, 255, 0.45)';
    ctx.beginPath(); ctx.roundRect(15, 315, 130, 70, 14); ctx.fill(); ctx.stroke();
    ctx.fillStyle = '#FFF'; ctx.font = 'bold 26px sans-serif';
    ctx.fillText('▼ 下', 80, 358);
    ctx.restore();
  }

// --- (オープニングボタン・下道/高速チュートリアル描画ブロックはそのまま維持) ---
  ...
  if (gameState === 'PLAYING_HIGHWAY' && showHighwayTutorial) {
    ...
    ctx.fillText('OK！発進する', canvas.width / 2, 348);
    ctx.restore();
  }
