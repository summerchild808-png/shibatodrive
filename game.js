...
//  game.js  Part 1 (前半・高速エンディング判定修正版)
// ===============================
...
let gameState = 'TITLE';
let currentStage = 1;
let lastPassedStage = 1; // 直前に進んでいた下道ステージ（初期値: 1）
...
function saveGame() {
  localStorage.setItem(SAVE_KEY, JSON.stringify({ stage: currentStage, lastPassed: lastPassedStage, money: scoreMoney }));
}
...
function init() {
  ...
  function processInputStart(canvasX, canvasY, buttonNum = 0) {
    ...
    if (gameState === 'TITLE') {
      ...
      if (saveData) {
        if (canvasX >= 140 && canvasX <= 440 && canvasY >= 340 && canvasY <= 410) {
          clearSaveData(); scoreMoney = 0; currentStage = 1; lastPassedStage = 1; gameState = 'STORY';
        } else if (canvasX >= 520 && canvasX <= 820 && canvasY >= 340 && canvasY <= 410) {
          currentStage = saveData.stage; lastPassedStage = saveData.lastPassed || saveData.stage; scoreMoney = saveData.money; gameState = 'SELECT_ROUTE';
        }
      } else {
        scoreMoney = 0; currentStage = 1; lastPassedStage = 1; gameState = 'STORY';
      }
      return;
    }
    ...
    // 🎬 ステージクリアカード表示後の進行判定（修正版）
    if (gameState === 'STAGE_CLEAR' && endCardTimer >= END_CARD_DELAY) {
      // 1. 下道で渋谷（4）をクリアした場合 ➔ エンディングへ
      if (currentStage === 4) {
        gameState = 'ENDING_STORY';
      }
      // 2. 高速（99）走行時：大阪クリア後（lastPassedStage >= 3）に高速を完走したら ➔ エンディングへ
      else if (currentStage === 99 && lastPassedStage >= 3) {
        gameState = 'ENDING_STORY';
      }
      // 3. 途中のステージクリアの場合 ➔ ステージを進めてルート選択画面へ
      else {
        if (currentStage !== 99) {
          lastPassedStage = currentStage;
          currentStage++;
        } else {
          // 高速を完走したら下道1ステージ分スキップ
          lastPassedStage++;
          currentStage = lastPassedStage + 1;
        }
        saveGame();
...
function selectShitadaRoute() {
  if (currentStage === 99) {
    currentStage = Math.min(4, lastPassedStage + 1);
  }
...
function draw() {
  ...
  if (gameState === 'STAGE_CLEAR') {
    ...
    if (currentStage === 2) clearImg = images.endCardFukuoka; // 広島（共用または設定）
    else if (currentStage === 3) clearImg = images.endCardOsaka;
    else if (currentStage === 4) clearImg = images.endCardShibuya; // 🏙️ 渋谷クリア！
  ...
  // ===============================
  if (gameState === 'ENDING_STORY') {
    safeDraw(images.endingStory, 0, 0, canvas.width, canvas.height);
  }

  // 🎉 ===============================
  //  エンディング演出 2：サンキューカード画面
  // ===============================
  if (gameState === 'THANK_YOU') {
    safeDraw(images.thankYouCard, 0, 0, canvas.width, canvas.height);
  }
