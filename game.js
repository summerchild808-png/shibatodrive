...
const imageSources = {
  ...
  obstacleTnt: 'obstacle_tnt.png', // 🧨 新・TNT爆弾画像
...
function init() {
  ...
  function processInputStart(canvasX, canvasY, buttonNum = 0) {
    // ⏸️ ポーズボタン（判定範囲: X: 720〜820, Y: 0〜80）
    ...
    // 🕹️ 下道プレイ中の操作（判定エリア拡大版）
    if (gameState === 'PLAYING') {
      if (canvasX > 700 && canvasY > 240) {
        handleAction();
      } else if (canvasX <= 220 && canvasY >= 240 && canvasY < 355) {
        moveLane(-1);
      } else if (canvasX <= 220 && canvasY >= 355) {
        moveLane(1);
      }
      return;
    }

    // 🏎️ 高速道路プレイ中の操作（判定エリア拡大版）
...
function updatePlayerAndObjects() {
  ...
  if (spawnTimer > spawnThreshold) {
    ...
    if (gameState === 'PLAYING_HIGHWAY') {
      ...
      else if (rand < 0.92) selectedType = 'obstacle_tnt'; // 🧨 マンホールからTNTに変更
      ...
      else if (selectedType === 'obstacle_tnt') { objData.y = LANES[spawnLane] + 10; objData.width = 50; objData.height = 50; } // 🧨 TNTサイズ
  ...
  for (let i = objects.length - 1; i >= 0; i--) {
    ...
    if (isSameLane && player.x < obj.x + obj.width && player.x + player.width > obj.x && (player.isGrounded || obj.type === 'orbis')) {
      if (obj.type === 'hitchhiker') {
        ...
        // 💥 TNT接触：即死＆ゲームオーバー
        life = 0;
        addPopup('💥 ＤＯＷＮ！！', '#FF0000', player.x + 20, player.y - 30);
        gameState = 'GAMEOVER';
...
function draw() {
  ...
  // 🏙️ 背景タウンの表示高さ調整（画面上部 Y:0 〜 道路の上端 Y:350）
  ...
  renderList.forEach(item => {
    if (item.isPlayer) {
      ...
      else if (item.type === 'obstacle_tnt') img = images.obstacleTnt; // 🧨 TNT画像
  ...
  if (gameState === 'GAMEOVER') {
    ...
    ctx.strokeText('💥 ＤＯＷN 💥', canvas.width / 2, 200); ctx.fillText('💥 ＤＯＷN 💥', canvas.width / 2, 200);
